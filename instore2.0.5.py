...
def webhook(sku, name, address, quantity, ctg):
    ...
    productImage = info[1]
    if ctg == 'CA':
        hook = Webhook('https://discord.com/api/webhooks/720101337957662780/NRm2Kd0SM9WmokNBvlM_gHMsD1cgSrr9_JggudA6I14-uy1-4YJtWDU9kHK6pG4E5itu')
    elif ctg == 'NY/NJ':
        hook = Webhook('https://discord.com/api/webhooks/720101888430964796/0tzMyXEtw8K_N4w1s_OEuywsaOb4KdkmWfPmS_Gt4QOILaNNVuQC2bpTs7193rLL7EDt')
    elif ctg == 'TX':
        hook = Webhook('https://discord.com/api/webhooks/720327986116558878/NrdktaWHsJZdWo7q0vRnZKLgPKIYsZyQLvcHg-qm6U4bsQHeHf5s5v6mTRZa81Vl8W06')
    elif ctg == 'NC':
        hook = Webhook('https://discord.com/api/webhooks/737050681516490773/VU_q-7AwwrBw0QJqis6v_5jIBgtzvLQ6oVoIJCwHxHO33FR1lP_XJ9kYkC2FEVuSP0YM')
    elif ctg == 'PA':
        hook = Webhook('https://discord.com/api/webhooks/737050842498334731/Qxho8UyZoK1gTUWa8XeMQgL_01iYldNVzqmk0CbsqnmUZwk2GHb1OcbKn59x7NNoEJM2')
    elif ctg == 'IL':
        hook = Webhook('https://discord.com/api/webhooks/737050943253643306/_Rz875PGMJexmitj5-TrjfUGoeFZFiuzIfZ2Kcdh1TGkNE3flhwGUhwh9I79o-rHgXMo')
    elif ctg == 'FL/GA':
        hook = Webhook('https://discord.com/api/webhooks/737051030109552660/AH4h36hB5dW0cpwRjyAUGgS26ZtnIhSAzAQUXLm931qIZGk8LOCLISBIGimbeqTq_fnt')
    elif ctg == 'DMV':
        hook = Webhook('https://discord.com/api/webhooks/737051138246967386/djfgdX4wwGMk336Ft6t_AGi6JRC3kf7_GG-daJtGRdZxbB3OWsN40KK3aypYKCVTT3GI')
    elif ctg == 'AZ':
        hook = Webhook('https://discord.com/api/webhooks/737051222824976525/4Hny5X7XnTF7Ju1aWHydFgppbLvHPH-vAJ1mmb3VW8zCIFnqmfjmh88IHRoOXXuRWi0V')
    elif ctg == 'NEW_ENGLAND':
        hook = Webhook('https://discord.com/api/webhooks/737053468967305258/Jdr-apz8eHLub-4BgFmYSW-yruTASWVAYsPOFOzEl8bjVAMry6LSRRKdcmbRYcBu9esJ')
    else:
        hook = Webhook('https://discord.com/api/webhooks/706970539071242292/6gH5YHCF3UNDz8fwoe2gsiE-7qhG3nNVBpnpiMVTIqttnUUwRdQXtr4F2ej7He3VfO84')
    embed = Embed(
    ...
    embed.set_footer(text='created by enrique#2519', icon_url='https://cdn.discord.com/avatars/267782036893204481/58904351e4e12eec9302b22cd6b209d9.webp?size=256')
...
def checkProduct(sku):
    ...
    monitor = requests.get("https://api.target.com/fulfillment_aggregator/v1/fiats/{}?key=ff457966e64d5e877fdbad070f276d18ecec4a01&nearby=80301-1123&limit=100&requested_quantity=1&radius=10000&include_only_available_stores=true&fulfillment_test_mode=grocery_opu_team_member_test".format(sku))
    res = monitor.json()
